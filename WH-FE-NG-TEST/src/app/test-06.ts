...
@Component({
    ...
    template : `
                <h2>User Review:</h2>
                <textarea class="textfield" placeholder="Write your Review" [(ngModel)]="review_input" (ngModelChange)="parseText()"></textarea>
                <br/><br/>
                <h3>Output:</h3>
...
At https://wallethub.com <b>bolded text</b>`
    ...
    ngOnInit() {
      this.parseText();
		...
		convertTextToAnchorTags(text){
			var regex = /(\b(http|https):\/\/([\w-]+\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?)/ig;
			return text.replace(regex, "<a target='_blank' href='$1'>$3</a>"); 
		}

		parseText (){
			this.review_content = this.convertTextToAnchorTags(this.review_input);
		}
