...
const writeToFile = async (fileName, data) => {
	try {
		await fs.outputFile(fileName, data, {flag: 'a'});
	} catch (err) {
		console.error(err)
	}
}

const findAgesAndWrite = async (data, age) => {
	data = data.split(', ');
	let count = 0;
	for (var i = 1; i < data.length; i += 2) {
		if (+data[i].split('=')[1] === age) {
			count += 1;
			await writeToFile('output.txt', data[i - 1] + '\n');
		};
	}
	console.log(`Age ${age} appears ${count} times`);
}

findAgesAndWrite(json.data, 32);
