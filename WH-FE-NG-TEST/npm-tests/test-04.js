...
const removeScriptTags = text => text.replace(/<script>[^]+<\/script>/gi, '');
...
app.post('/save',(request,reply)=>{
    ...
    // purify the inputs here
		userInput.firstname = removeHTMLTags(removeScriptTags(body.firstname));
		userInput.lastname = removeHTMLTags(removeScriptTags(body.lastname));
